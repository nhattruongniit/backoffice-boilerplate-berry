...
const NavCollapse = ({ menu, level, parentId }: any) => {
  ...
  const { mode, menuOrientation, borderRadius, drawerOpen } = useAppSetting();
  ...
  const handleClickMini = (event: any) => {
    setAnchorEl(null);
    if (drawerOpen) {
      setOpen(!open);
      ...
      {!isHorizontal ? (
        <>
          <ListItemButton
            ...
            selected={isSelected}
            {...((!drawerOpen && { onMouseEnter: handleClickMini, onMouseLeave: handleClosePopper }) as any)}
        ...
        >
          ...
          />
          {openMini ? <KeyboardArrowRightIcon /> : <KeyboardArrowDownIcon />}
